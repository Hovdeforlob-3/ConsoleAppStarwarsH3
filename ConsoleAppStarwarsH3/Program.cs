...
namespace ConsoleAppStarwarsH3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var item in Opg16(planets))
            {
        ...
        static IEnumerable<Planet> Opg1(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.Name.StartsWith("M"));

            return planetslst;
        }

        static IEnumerable<Planet> Opg2(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.Name.Contains("y") || p.Name.Contains("Y"));

            return planetslst;
        }

        static IEnumerable<Planet> Opg3(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.Name.Length > 9 && p.Name.Length < 15);

            return planetslst;
        }

        static IEnumerable<Planet> Opg4(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.Name[1].Equals('a') && p.Name.EndsWith("e"));

            return planetslst;
        }

        static IEnumerable<Planet> Opg5(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.RotationPeriod > 40).OrderBy(p => p.RotationPeriod);

            return planetslst;
        }

        static IEnumerable<Planet> Opg6(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.RotationPeriod > 10 && p.RotationPeriod < 20).OrderBy(p => p.Name);

            return planetslst;
        }

        static IEnumerable<Planet> Opg7(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.RotationPeriod > 30)
                                           .OrderBy(p => p.Name).ThenBy(p=> p.RotationPeriod);

            return planetslst;
        }

        static IEnumerable<Planet> Opg8(List<Planet> planets)
        {
            var planetslst = planets.Where(p => (p.RotationPeriod < 30 || p.SurfaceWater > 50) && p.Name.Contains("ba"))
                                           .OrderBy(p => p.Name).ThenBy(p => p.RotationPeriod).ThenBy(p => p.SurfaceWater);

            return planetslst;
        }

        static IEnumerable<Planet> Opg9(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.SurfaceWater > 0).OrderByDescending(p => p.SurfaceWater);

            return planetslst;
        }

        static IEnumerable<Planet> Opg10(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.Diameter > 0 && p.Population > 0)
                                           .OrderBy(p => (4 * Math.PI * p.Diameter) / p.Population);
 

            return planetslst;
        }

        static IEnumerable<Planet> Opg11(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.RotationPeriod > 0);

            var List = planets.Except(planets, new NameComparer());

            return List;
        }

        static IEnumerable<Planet> Opg12(List<Planet> planets)
        {
            var planetslstWhereAorSisIn = planets.Where(p => p.Name.StartsWith("A") || p.Name.EndsWith("s"));

            var planetslstWhereTisRainforests = planets.Where(p => p.Terrain != null &&  p.Terrain.Contains("rainforests"));


            var planetslst = planetslstWhereAorSisIn.Union(planetslstWhereTisRainforests);

            return planetslst;
        }

        static IEnumerable<Planet> Opg13(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.Terrain != null && (p.Terrain.Contains("desert") || p.Terrain.Contains("deserts") || p.Terrain.Contains("rocky deserts")));

            return planetslst;
        }

        static IEnumerable<Planet> Opg14(List<Planet> planets)
        {
            var planetslst = planets.Where(p => p.Terrain != null && (p.Terrain.Contains("swamp") || p.Terrain.Contains("swamps")))
                                    .OrderBy(p => p.RotationPeriod).ThenBy(p => p.Name); 
                                    
            return planetslst;
        }

        static IEnumerable<Planet> Opg15(List<Planet> planets)
        {
        ...
        static IEnumerable<Planet> Opg16(List<Planet> planets)
        {

            var planetslst = planets.Where(p => Regex.IsMatch(p.Name, @"([kk ll rr nn])\1")).OrderByDescending(p => p.Name);
